...
public:
    int maxJumps(vector<int>& arr, int d) {
        ...
        for(auto i: views::iota(0, n) | views::reverse) {
            ...
            r[i] = min(st.top(), i + d + 1);
            st.push(i);
        }
        SegmentTree<int> ST(vector<int>(n, 0), [](int a, int b) {return max(a, b);} );
        ranges::iota(idx, 0);
        ranges::sort(idx, std::less<int>(), [&](int x) {return arr[x];});
        for(auto i: idx) {
            int mx = ST.query(l[i] + 1, r[i]);
            res = max(mx + 1, res);
            ST.update(i, mx + 1);
        }
        return res;
